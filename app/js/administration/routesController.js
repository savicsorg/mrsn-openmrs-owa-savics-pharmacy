angular.module('RoutesController', []).controller('RoutesController', ['$scope', '$state', '$rootScope', '$mdToast', 'openmrsRest', '$mdDialog', '$translate', '$q', function ($scope, $state, $rootScope, $mdToast, openmrsRest, $mdDialog, $translate, $q) {
        ...
        $scope.getAllRoute = function () {
            ...
            openmrsRest.getFull($scope.resource + "/route?limit=" + $scope.query.limit + "&startIndex=" + $scope.query.startIndex).then(function (response) {
                $scope.loading = false;
                if (response.results.length >= 1) {
                    $scope.routes = response.results;
                }
                openmrsRest.get($scope.resource + "/route/count").then(function (response) {
                    if (response.count) {
